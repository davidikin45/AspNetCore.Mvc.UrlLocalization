...
namespace AspNetCore.Mvc.UrlLocalization
{
    ...
    //IApplicationModelConvention is the only one called when adding to Options.Conventions. IControllerModelConvention and IActionModelConvention are only called when used on attributes. 
    public class UrlLocalizationConvention : IApplicationModelConvention, IControllerModelConvention, IActionModelConvention, IPageRouteModelConvention
    {
        ...
        public void Apply(PageRouteModel model)
        {
            ...
            //Page has Attribute Route
            var matchedSelectors = model.Selectors.Where(x => x.AttributeRouteModel != null).ToList();
            if (matchedSelectors.Any())
            {
                foreach (var selectorModel in matchedSelectors)
                {
            ...
            foreach (var newSelector in newSelectors)
            {
                model.Selectors.Insert(0, newSelector);
            }
        }
    }
}
